 ...
 */
 ...
 Lantern.controller('previsionalCountedCtrl', ['$scope', '$rootScope', '$filter', '$location', '$cookies', 'Farmer', 'User', 'Project','ApiRest',
 function($scope, $rootScope, $filter, $location, $cookies, Farmer, User, Project, ApiRest) {
   ...
   $scope.isActiveActions = true;
   $scope.isActiveStudios = true;
  ...
  ApiRest.get('/valuelists/values/', {
   ...
   $scope.projectActionSelected = function() {
    $scope.activeFarmers =[]
    if($scope.selectedActions.length > 0) {
      if ($scope.selectedStudios.length > 0) {
        if ($scope.isActiveActions) {
          if ($scope.isActiveStudios) {
            $scope.activeFarmers = $scope.farmers.filter(farmer => $scope.selectedActions.some(action => action.id === farmer.vett_value + ' / ' + farmer.vat_value) && $scope.selectedStudios.some(studio => studio.id === farmer.audit))
          } else {
            $scope.activeFarmers = $scope.farmers.filter(farmer => $scope.selectedActions.some(action => action.id === farmer.vett_value + ' / ' + farmer.vat_value) && !$scope.selectedStudios.some(studio => studio.id === farmer.audit))
          }
        } else {
        ...
        if ($scope.isActiveActions) {
          $scope.activeFarmers = $scope.farmers.filter(farmer => $scope.selectedActions.some(action => action.id === farmer.vett_value + ' / ' + farmer.vat_value))
        } else {
          $scope.activeFarmers = $scope.farmers.filter(farmer => !$scope.selectedActions.some(action => action.id === farmer.vett_value + ' / ' + farmer.vat_value))
        }
      }
      ...
      if ($scope.selectedStudios.length > 0) {
        if ($scope.isActiveStudios) {
          $scope.activeFarmers = $scope.farmers.filter(farmer => $scope.selectedStudios.some(studio => studio.id === farmer.audit))
        } else {
          $scope.activeFarmers = $scope.farmers.filter(farmer => !$scope.selectedStudios.some(studio => studio.id === farmer.audit))
        }
      } else {
   ...
   function getFarmerByRange(user_id, project, action) {
     ...
     Farmer.getPrevisioFarmersBy({
       ...
       angular.forEach(farmers, function(farmer) {
         ...
         var farmerAction = {}
         farmerAction.id = farmer.vett_value + ' / ' + farmer.vat_value
         farmerAction.value = farmer.vett_value + ' / ' + farmer.vat_value
         farmerActions.push(farmerAction)
         var farmerStudio = {}
         farmerStudio.id = farmer.audit
         farmerStudio.value = farmer.audit
         farmerStudios.push(farmerStudio)
         ...
         if(farmer.start_time != null && farmer.end_time != null){
          ...
          farmer.estimateTime = (hour_estimate)+'h'+(minute_estimate)
         }
       })
       ...
       farmerStudios.forEach(obj => {
        if (!sids.includes(obj.id)) {
          var studio = {}
          studio.id = obj.id
          studio.value = obj.value
          $scope.studioNames.push(studio);
          sids.push(obj.id)
        }
       })
       $scope.calculInProgress--
