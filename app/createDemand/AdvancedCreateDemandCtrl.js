...
*/
Lantern.controller('AdvancedCreateDemandCtrl', ['$filter', '$rootScope', '$scope', 'ApiRest', 'Request', 'RequestService', 
'FileUploader', 'ProductService', '$q', 'ValueListService', 'StepsList', 'ngDialog', 'MediaItems', 'Return',
 'Comment', 'NotificationService', 'TableauSuivi', 'PersonsService', 'Session',
 'WorkflowHelperService','dataSync', 'PaoService','ProjectsService', 'HelperService','RoomService',
  function($filter, $rootScope, $scope, ApiRest, Request, RequestService, FileUploader, ProductService, $q, 
    ValueListService, StepsList, ngDialog, MediaItems, Return, Comment, NotificationService, 
    TableauSuivi, PersonsService,Session, WorkflowHelperService,dataSync, PaoService, ProjectsService, HelperService, RoomService) {
    $scope.form = {};
    ...
    $scope.dateStartEnd = [];
    $scope.uploader = {};
    $scope.durationReelAllIsOk = false
    ...
    */
    $scope.projectType = {}
    $scope.projectTypeSelected = null
    $scope.changeProjectType = function (value) {
      ProjectsService.updateProject($scope.getProject().id, { type_id: value.id } , function (result) {
        getMainProduct(getProducts()).subproject.project = result
      }, function (error) {})
    }
    let rest = {};
    let service = {};
    ...
    */
    ...
    const getPlanning = () => plannings[requestToCreate.planning_id];
    ...
    const getEtapesActions = () => {
      if (workflowsChosen.length > 0) {
        if (workflowsChosen[0].workflow_type_id == 1) {
          ...
          angular.forEach(etapesActions[1], function (action) {
            let insertAction = true;
            if (action.service === "belgique") {
              if (!(workflowsChosen[0].dub_place === "Hybride" || workflowsChosen[0].dub_place === "Belgique")) {
                insertAction = false;
              }
            }
            if (action.etape.name === "enregistrement") {
              if (!(workflowsChosen[0].dub_place === "Hybride" || workflowsChosen[0].dub_place === "France" 
                  || workflowsChosen[0].dub_place === "Italia" || workflowsChosen[0].dub_place === "USA" || workflowsChosen[0].dub_place === null)) {
                insertAction = false;
              }
            }
            if (insertAction) {
    ...
    const createRequest = async data => {
      if (isSendRequestsEnabled()) {
        ...
        data.requestToCreate.dateStartEnd.forEach(function (item) {
          if (item.end_time_h && item.end_time_h.value) {
            item.end_time_h = item.end_time_h.value
          }
          if (item.start_time_h && item.start_time_h.value) {
    ...
    const autoSelectEtapeAction = () => {
      if (etape_action_id != null && workflowsChosen.length > 0) {
        let etapesActions = getEtapesActions();
        let action = $filter('filter')(etapesActions, {
          'id': etape_action_id
        }, true);
        if (action.length > 0) {
    ...
    /**
     * check duration and reel
     *    
     * 
     * 
     **/ 

    const checkDurationAndReel = function () {
      $scope.durationReelAllIsOk = true
    }
    ...
    const init = () => {
      PaoService.getDubbingSteps(function (dubbingSteps) {
        $scope.dubbingSteps = dubbingSteps
      })
    ...
    $scope.ReelAllIsOk = false
    $scope.durationAllIsOk = false

    $scope.hasDurationAndReel = function (what) {
      // un peu longuet, mais pas le temps de faire mieux
      // vérifie uniquement sur les étapes enregistrement et mixage
      if ($scope.getActionChosen && $scope.getActionChosen() && ($scope.getActionChosen().etape.name == 'enregistrement' || $scope.getActionChosen().etape.name == 'mixage')) {
        if ($scope.getWorkflowsChosen()[0].doublage_type_id == 1 && $scope.getWorkflowsChosen()[0].exploitation_id == 1) {
          // vérifie  si le nombre de bobine est indiqué
          $scope.durationAllIsOk = true
          if ($scope.getProducts()[0].number_reel) {
            $scope.ReelAllIsOk = true
            return true
          } else {
            $scope.ReelAllIsOk = false
            return false
          ...
          // vérifie si la durée est indiquée
          $scope.ReelAllIsOk = true
          if ($scope.getProducts()[0].duration) {
            $scope.durationAllIsOk = true
            return true
          } else {
            $scope.durationAllIsOk = false
    ...
    $scope.openDateChoiceModal = function () {
      let dubbingStep = PaoService.getDubbingStep(request.action_type.etape_type.name)
      const dataSent2modal = {
            requestCreation: true, // nouvelles requetes à créer
            original_dates: [], 
            products: $scope.getProducts(), 
            action_id: getEtapeActionId(), 
            actionType: getActionChosen(),
            dubbingStep: dubbingStep, 
            workflows : $scope.getWorkflowsChosen(), 
      ...
      dialReturn.closePromise.then(function (data) {
        // mettre à jour le tableau $scope.dateStartEnd, ensuite le processus suit son cours
        ctrl.initPopup()
      })
    }  
